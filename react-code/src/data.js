
export const QUOTES = [
    {
        id: 1,
        text: "A sua riqueza cresce na medida que você cresce"
    },
    {
        id: 2,
        text: "Se você quer conhecer um negócio, entre nele!"
    },
    {
        id: 3,
        text: "Se eles conseguiram, eu também consegues!"
    },
    {
        id: 4,
        text: "Por causa de atenção a pessoa tende a fazer coisas idiotas para consegui-la"

        
    },
    {
        id: 5,
        text: "Conhecimento só vale quando é aplicado"
    },
    {
        id: 6,
        text: "Conhecimento só vale quando é aplicado"
    },
    {
        id: 7,
        text: "A excelência, portanto, não é um ato, mas sim um hábito"
    },
    {
        id: 8,
        text: "Seus pensamentos criam suas experiência de vida"
    },
    {
        id: 9,
        text: "Por traz de cada conquista impossível há um sonhador de sonhos impossíveis"
    },
    {
        id: 10,
        text: "Se você não sabe para aonde estás indo, acabarás em qualquer lugar!"
    },
]

export const SUCCESS_FILES = [
    {
        id: 1,
        text: `As pessoas ricas acreditam na seguinte ideia:
 “Eu crio a minha própria vida.”
 As pessoas de mentalidade pobre acreditam na
 seguinte idéia: “Na minha vida, as coisas acontecem.”`
    },
    {
        id: 2,
        text: `As pessoas ricas entram no jogo do dinheiro para ganhar.
 As pessoas de mentalidade pobre entram no jogo
 do dinheiro para não perder.`
    },
    {
        id: 3,
        text: `As pessoas ricas assumem o compromisso de serem ricas.
 As pessoas de mentalidade pobre gostariam de ser ricas.`
    },
    {
        id: 4,
        text: `As pessoas ricas pensam grande.
 As pessoas de mentalidade pobre pensam pequeno.`
    },
    {
        id: 5,
        text: `As pessoas ricas focalizam oportunidades.
 As pessoas de mentalidade pobre focalizam obstáculos.`
    },
    {
        id: 6,
        text: `As pessoas ricas admiram outros
 indivíduos ricos e bem-sucedidos.
 As pessoas de mentalidade pobre guardam
 ressentimento de quem é rico e bem-sucedido.`
    },
    {
        id: 7,
        text: ` As pessoas ricas buscam a companhia
 de indivíduos positivos e bem-sucedidos.
 As pessoas de mentalidade pobre buscam a companhia
 de indivíduos negativos e fracassados.`
    },
    {
        id: 8,
        text: `As pessoas ricas gostam de se promover.
 As pessoas de mentalidade pobre não apreciam
 vendas nem autopromoção.`
    },
    {
        id: 9,
        text: `As pessoas ricas são maiores do que os seus problemas.
 As pessoas de mentalidade pobre são menores
 do que os seus problemas.`
    },
    {
        id: 10,
        text: `As pessoas ricas são excelentes recebedoras.
 As pessoas de mentalidade pobre
 são péssimas recebedoras.`
    },
    {
        id: 11,
        text: `As pessoas ricas preferem ser
 remuneradas por seus resultados.
 As pessoas de mentalidade pobre preferem
 ser remuneradas pelo tempo que despendem.`
    },
    {
        id: 12,
        text: `As pessoas ricas pensam: “Posso ter as duas coisas.”
 As pessoas de mentalidade pobre pensam:
 “Posso ter uma coisa ou outra.”`
    },
    {
        id: 13,
        text: ` As pessoas ricas focalizam o seu patrimônio líquido.
 As pessoas de mentalidade pobre focalizam o
 seu rendimento mensal.`
    },
    {
        id: 14,
        text: ` As pessoas ricas administram bem o seu dinheiro.
 As pessoas de mentalidade pobre administram
 mal o seu dinheiro.`
    },
   
    {
        id: 15,
        text: `As pessoas ricas põem o seu dinheiro para dar duro para elas.
 As pessoas de mentalidade pobre dão duro pelo seu dinheiro.`
    },
   
    {
        id: 16,
        text: `As pessoas ricas agem apesar do medo.
 As pessoas de mentalidade pobre
 deixam-se paralisar pelo medo.`
    },
   
    {
        id: 17,
        text: ` As pessoas ricas aprendem e se aprimoram o tempo todo.
 As pessoas de mentalidade pobre acreditam
 que já sabem tudo.`
    },
   
]